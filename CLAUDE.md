...
- Codex 리뷰의 findings 중 버그가 아니라 개선 제안 성격인 것은, 곧바로 구현하기 전에 "이 프로젝트에 지금 실제로 필요한가"를 먼저 판단한다. 특히 규칙(Rule) 문서에 대한 제안은 문자 그대로 규칙화하기보다, 실제 사례가 있는지 먼저 확인한다.
- fix 이후에도 같은 위치/내용에 대한 지적이 반복되면, 세부 표현을 계속 고치는 대신 그 내용 자체의 범위나 존재 이유를 재검토하고 사용자에게 보고한다.
