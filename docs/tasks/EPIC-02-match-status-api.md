...
회사별 매칭 계산 상태(`IN_PROGRESS`/`DONE`/`FAILED`/미시작)를 조회하는 API와, `FAILED`이거나 `IN_PROGRESS`가 5분 넘게 갱신되지 않은(죽은 락) 상태일 때 프로필 재전송 없이 재계산만 다시 트리거하는 재시도 API를 추가한다.
...
- [x] `POST /api/companies/me/match-status/retry`가 상태가 `FAILED`이거나, `IN_PROGRESS`가 5분 넘게 갱신되지 않았을 때만 락을 재획득하고 재계산을 다시 트리거한다 (상태 확인과 락 획득은 CAS 쿼리 하나로 원자적으로 처리해 조회-갱신 사이 경쟁 상태를 없앤다 — Codex 리뷰 반영)
- [x] 신선한 `IN_PROGRESS`이거나 `DONE`인데 재시도를 호출하면 전용 에러 코드로 거부된다 (DONE은 일반 `acquireLock`과 달리 재시도 전용 CAS를 통과하지 못한다)
...
- [x] `MatchCalculationStatusRepository`에 재시도 전용 CAS 쿼리(`acquireRetryLock`) 추가, 락 유효기간 상수(`MatchCalculationStatus.LOCK_STALE_MINUTES`)를 공용화
- [x] 기존 이벤트/리스너 관련 테스트 클래스명·참조 갱신
- [x] 신규 Service/Controller/Repository 단위·통합 테스트 작성 (재시도 CAS의 FAILED/DONE/신선한 IN_PROGRESS/낡은 IN_PROGRESS 4가지 경계 포함)
