...
- [x] `./gradlew bootRun`으로 Spring Boot 앱이 정상 실행된다
- [x] Docker Compose로 로컬 PostgreSQL이 실행되고 앱이 DB에 연결된다
- [x] Flyway가 기동 시 실행되고 `flyway_schema_history` 테이블이 생성된다
- [x] `GET /api/health` 요청 시 공통 Wrapper 형태로 200 응답이 반환된다
- [x] `ApiException` 발생 시 `@RestControllerAdvice`에서 공통 Wrapper 형태 에러 응답이 반환된다
...
- [x] `backend/` 하위에 Spring Boot 프로젝트 생성
...
- [x] `@RestControllerAdvice` 전역 예외 처리기 구현
  - `ApiException` 처리
...
- [x] `application.yml` 작성
  ...
  - Flyway 활성화
- [x] `@ConfigurationProperties` 설정 클래스 기본 구조 작성
  - `AppProperties` (app.* prefix)
  - `G2bProperties` (app.g2b.* prefix, 나라장터 API 키 등)
- [x] Docker Compose 작성 (`infra/docker-compose.local.yml`)
  - PostgreSQL 서비스
  - 환경 변수 설정
- [x] Flyway 초기 마이그레이션 파일 추가
  - `V1__init.sql` (빈 파일 또는 extensions 설정)
- [x] `GET /api/health` 헬스체크 엔드포인트 구현
  - 공통 Wrapper 적용 확인용
