...
2. **다음을 모두 만족하면 같은 PR에 포함한다** (사용자 승인 후, PR 설명에 "범위 확장: {내용}" 명시):
   - 이번 Task/PR이 직접 만든 회귀다.
   - 변경 규모가 작다 (필드 추가 등 non-breaking, 기존 계약을 깨지 않음).
   - 별도 설계 합의 없이 판단 가능하다.
3. **하나라도 해당하면 GitHub Issue로 분리한다** (이번 PR에는 known limitation으로 명시하고 원래 범위만 완료):
   - 이전부터 있던 별개 이슈다.
