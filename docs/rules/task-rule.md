...
구현 계획은 사용자가 실제로 검토·판단할 수 있는 형태로 제시한다. 문장이 매끄럽게 읽힌다고 그 설계가 맞다는 뜻은 아니다 — "이해된다"와 "동의한다"는 다르다. 아래 항목을 한 번에 몰아서 승인받지 않고 순서대로 제시한다.

1. **불변조건**: 이 태스크가 지켜야 할 결과 계약을 구현 방법보다 먼저 확정한다 (예: "실패해도 이미 처리된 다른 항목의 결과는 되돌리지 않는다"). 사소한 태스크는 한두 줄로 끝나도 된다.
2. **결정 목록**: 여러 방식으로 구현 가능하거나 틀렸을 때 되돌리기 어려운 지점을 "결정 / 이유 / 틀렸을 때 나타나는 현상 / 대안"으로 짚는다. Task 문서에 없어 가정이 필요한 부분(기존 "불확실한 지점")도 여기 포함한다. 실행 순서·트랜잭션·동시성·인증처럼 결과가 순서/타이밍에 좌우되는 지점은, 결론부터 말하지 않고 사용자가 먼저 예측하게 한 뒤 비교하는 방식(predict-first)을 우선한다. 결정할 지점이 없으면 이 항목 자체를 생략한다.
3. **시나리오** (분기·실패 경로가 있을 때만): 정상 흐름 외 실패/부분 실패/동시 실행 등 여러 경로가 있으면 경로별 최종 상태를 표로 정리한다. 분기가 없으면 생략한다.
4. **작성할 테스트 목록**: 메서드 이름이 아니라 "무엇을 증명하는가"로, 요구사항 단위로 나열한다 (예: "중복 공고는 저장되지 않는다"). Spring Boot는 `docs/rules/springboot/testing-rule.md`, Frontend는 `docs/rules/frontend/testing-rule.md`의 기준에 따라 대상 여부와 (Spring Boot의 경우) 단위/통합 구분을 표시. 테스트 생략 허용 기준(Spring Boot는 `self-review-rule.md`의 단순 CRUD/설정/Mapper, Frontend는 `frontend/testing-rule.md` §4-2)에 해당하면 "테스트 생략: {사유}"로 명시하고, 생략한 화면 흐름은 Frontend Task에 한해 `docs/qa/` 시나리오 문서 작성 여부도 함께 표시.
5. **변경/생성할 파일 목록**: 스코프 확인용이라 가볍게 나열한다.

- 개발 도중 계획에 없던 파일을 수정해야 하거나, 계획한 구현 방식이 실질적으로 달라지는 상황이 생기면 진행을 멈추고 사용자에게 보고 후 승인을 받는다. 처음엔 단순해 보여 2번 결정 목록 자체를 생략했더라도, 구현 중 다른 계층 구조나 실행 흐름이 필요해지는 것도 여기 해당한다 — "결정 목록에 있던 항목이 바뀌는 경우"로 좁혀 읽지 않는다.
- 구현 완료 후, 트랜잭션/스레드 경계가 있는 diff는 실행 흐름 순서로 사용자와 함께 읽는다 (2번 항목에 그런 결정이 있었던 경우).
