...
2. **push 직후 바로 `gh pr comment {PR번호} --body "@codex review"`를 실행한다.** (2026-07-14 변경: 예전에는 "자동 트리거를 몇 분 기다렸다가 반응 없으면 수동 트리거"였는데, 이 대기 시간 자체가 사용자 입장에서 불필요한 확인 왕복이라 판단해 순서를 바꿨다. PR을 처음 열 때의 자동 리뷰는 그대로 살아있으니 중복 리뷰가 걱정되면 이 커맨드 실행 자체는 그대로 두되, "기다려야 하나?"는 더 이상 고민하지 않는다.)
3. **세 엔드포인트를 모두 확인한다.** Codex는 findings가 있으면 인라인 리뷰 코멘트를, 실행 오류(트리거 실패·quota 등)가 있으면 PR 대화 코멘트를 남기고, **findings가 없으면 코멘트 없이 PR에 👍(`+1`) 리액션만 남긴다.**
...
4. 수 분(10분 이상) 기다려도 위 기준의 새 리뷰 코멘트/대화 코멘트가 없으면, 2번에서 이미 트리거를 보냈으므로 재트리거하지 않고 조금 더 기다린 뒤 다시 3번 방식으로 재확인한다.
5. findings나 실행 오류가 있으면 처리 후 다시 1번으로 돌아간다. 충분히 기다려도(20~30분) 새 리뷰 코멘트/대화 코멘트가 끝내 없다면:
   - 이 PR에 봇의 `+1` 리액션이 **한 번도 없었다면** — 트리거를 보냈는데도 응답이 전혀 없었다는 뜻이므로, 종료하지 말고 사용자에게 상황을 보고하고 GitHub PR 페이지에서 직접 확인해달라고 요청한다.
   - 이 PR에 봇의 `+1` 리액션이 **이미 있었다면** — "이번 push도 클린해서 조용한 것"과 "실행 자체가 안 된 것"을 API로는 구분할 수 없는 한계 상황이다. 자동으로 종료하지 않고, 이 사실을 사용자에게 알린 뒤 GitHub PR 페이지에서 최신 커밋 옆 리뷰 상태를 직접 확인해달라고 요청한다.
