...
├── constants/    # 정적 선택지 상수 (지역, 입찰방식 등 — 3-1 참고)
...
### 3-1. 정적 선택지 데이터의 출처 판단

select box, 드롭다운 등에서 쓰는 고정 값 목록은 데이터 소유 주체를 기준으로 위치를 정한다.

- **외부 기관/API가 정의한 고정 분류 체계** (예: 나라장터 입찰방식/계약방식, 행정구역) — 우리가 값을 추가·편집할 일이 없고 변경 주기가 매우 길다. `src/constants/`에 상수 배열로 둔다. 값의 근거(원본 스펙 문서 등)를 주석으로 남긴다.
- **제품이 직접 소유·편집하는 도메인 데이터** (예: 기술 태그, 사업 분야처럼 향후 운영자가 추가·삭제할 수 있는 값) — 백엔드 `/api/codes/*`로 조회하고 `src/hooks/`의 React Query 훅으로 감싼다. 프론트 재배포 없이 값을 바꿀 수 있어야 하는 데이터이기 때문이다.
- 둘 중 어디에 속하는지 애매하면 API 방식을 기본값으로 택한다. 상수로 시작했다가 나중에 API로 옮기는 것보다, 처음부터 API로 시작했다가 필요 없으면 상수로 낮추는 쪽이 비용이 적다.
