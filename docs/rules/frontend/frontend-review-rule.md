...
- [ ] `testing-rule.md` §4-1 기준(계산/분기가 있는 순수 함수, validator, interceptor 로직, 훅 내부 계산)에 해당하는 코드에 Vitest 테스트를 작성했다
- [ ] `npm test`, `npm run lint`가 통과한다
- [ ] 테스트를 생략한 화면 흐름은 `docs/qa/`에 시나리오 문서(사전조건/절차/기대결과)로 남겼다

### 검증

- [ ] `docs/qa/`에 남긴 골든 패스와 주요 에러 상태(검증 실패, API 실패, 빈 상태) 시나리오를 Playwright MCP로 직접 확인했다 — 타입체크/린트/단위테스트로 안 잡히는 렌더링·포커스·레이아웃 버그가 있다
