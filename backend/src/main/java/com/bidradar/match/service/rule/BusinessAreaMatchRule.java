...
@Component
public class BusinessAreaMatchRule implements ScoreRule {

    private static final String RULE_NAME = "businessAreaMatch";
    ...
    public ScoreResult calculate(BidNotice bid, Company company, CompanyProfileContext profile) {
        ...
        List<String> matched = profile.businessAreaNames().stream()
                .filter(searchText::contains)
                .sorted()
                .toList();

        if (matched.isEmpty()) {
            return new ScoreResult(RULE_NAME, 0, MAX_SCORE, "보유 사업 분야와 일치하는 내용 없음", matched);
        }
        return new ScoreResult(RULE_NAME, MAX_SCORE, MAX_SCORE,
                "보유 사업 분야 일치: " + String.join(", ", matched), matched);
    }
